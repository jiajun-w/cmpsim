...
#	          Each core has its own independent L1 but all
#		  share same L2 cache
#		+ The following simulator also takes care of 
#		  invalidation
...
def printSummary():
	...
	print "|                Hits:", repr(g2.hits)
	print "|              Misses:", repr(g2.misses)
	print "|            Hit Rate: %3.2f" % (g2.hits * 100.0/ (g2.hits + g2.misses))
	print "o--------------------------o"
...
#Setting up independent L1 cache
	i=0
	for i in range (0,4):	
		L1.append(g1(Cache_l1()))

#Setting up a shared L2 cache
		g2.cache = Cache_l2()
...
# and fill the data from L2/memory
			...
			l2_cacheHit = g2.cache.checkCacheHit(address)
			if l2_cacheHit > 0:
				g2.cache.incrementLRU(address)
				g2.hits+=1
			else:
				g2.cache.storeLRUBlock(address)
				g2.misses+=1
				tm=0
				for tm in range (0,4):
					L1[tm].cache.invalidate(address)
