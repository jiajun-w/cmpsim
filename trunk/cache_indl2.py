...
#	       Each core has its own independent L1 and L2 cache
...
def main():
	...
	if not len(sys.argv) == 7: # Check for command line arguments
		print "Usage: %s [ cache size L1 kB ] [ associativity L1 kB ] [ cache size L2 kB ] [ associativity L2 kB ] [ trace filename ] [ block_size B ]\n" % \
...
# and fill the data from L2/memory
			...
			l2_cacheHit = L2[file_ext].cache.checkCacheHit(address)
			if l2_cacheHit > 0:
				L2[file_ext].cache.incrementLRU(address)
				L2[file_ext].hits+=1
			else:
				L2[file_ext].cache.storeLRUBlock(address)
				L2[file_ext].misses+=1
